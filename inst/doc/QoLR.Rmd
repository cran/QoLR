...
references:
- id: fayers1999eortc
  title: EORTC QLQ-C30 Scoring Manual
  author:
  - family: Fayers P, et al.
  type: article-journal
  publisher: EORTC
  issued:
    year: 1999
    
- id: aaronson1993european
...
- id: hamidou2011time
  title: Time to Deterioration in Quality of Life Score as a Modality of Longitudinal Analysis in Patients with Breast Cancer
  author: 
  - family: Hamidou Z, et al.
  type: article-journal
...
- id: bonnetain2010time
  title: Time Until Definitive Quality of Life Score Deterioration as a Means of Longitudinal Analysis for Treatment Trials in Patients with Metastatic Pancreatic Adenocarcinoma
  author: 
  - family: Bonnetain F, et al.
  type: article-journal
...
The first application of the `QoLR` package is the estimation of the scores of most of the EORTC health-related quality of life (HRQOL) questionnaires like the QLQ-C30 cancer specific questionnaire [@aaronson1993european]. The method to calculate the scores of the EORTC HRQOL questionnaire is defined in the EORTC scoring manual [@fayers1999eortc]. All the modules implemented in the `QoLR` package are described in the next Table 1.
...
The first argument of the function `scoring.QLQC30` and other functions for scoring is the name of the dataset with the items answers to the questionnaire (`X` parameter). The patient's identification number can be specified in `id` parameter. A time identification number can also be specified in `time` parameter if a longitudinal assessment of HRQoL is performed.  
The items have to be named `q1` to `qi` for the QLQ-C30 (i=30), QLQ-C15-PAL (i=15) and IN-PATSAT32 (i=32) questionnaire. For all other supplementary modules, items have to be named `q31` to `qi`, because these modules have to be administered in conjunction to the QLQ-C30 core questionnaire.
...
The result is a data frame: each variable corresponds to a HRQOL score. The name of the scores are the same than those used in the EORTC scoring manual [@fayers1999eortc]. If a patient's identification number and/or a time's identification number was (were) specified in the `id` parameter and `time` parameter respectively, then this (these) variable(s) is (are) replicated in the data frame obtained.
...
The second application of the `QoLR` package is the estimation of the time to HRQoL score deterioration [@anota2013time]. Both time to first deterioration [@hamidou2011time] and time until definitive deterioration [@bonnetain2010time] were implemented.
...
The function `TTD` alows to estimate the time to first deterioration [@hamidou2011time]. Several definitions were implemented depending on the reference score, the minimal clinically important difference, including or not patients with no baseline score and/or no follow-up score as an event, including or not death as an event. 
...
1. Item Patient's identification number;
...
`order` | = 1 (default) /= 2
`no_baseline` | = `"censure"` (default)/= `"event"` /= `"excluded"`
`no_follow` | = `"censure"` (default)/= `"event"`
...
* a variable called `time` and equals to the time to censure or the time to the deterioration _in months_.

Like both variables `event` and `time` are created for each score treated and each definition of TTD, we added the name of the corresponding score as a suffix. As example, if `score = c("score1","score2")`.
Then four variables are created: `event.score1`, `time.score1`, `event.score2` and `time.score2`.
...
Then we reorganized the obtained `dataqol_final` dataset in order that the `date` variable appeared in the third position:
...
To begin, we can study the time to deterioration as compared to the baseline `QL` score (`ref.init = "baseline"`) with at least 5-point MCID (`MCID = 5`) and considering patients with no baseline or with no follow-up measure censoring at baseline or just after baseline (`no_baseline = "censure"` and `no_follow = "censure"`). 
...
Like the values of these parameters are the default values except for the MCID, we do not need to specify their values and the function can be applied as follows:
...
The result is a data frame with the identification number of patients (`Id`), the time to deterioration or to censure in months (`time.QL`) and a dummy variable (`event.QL`) indicating if the patient is deteriorated (`event.QL =1`) or not (`event.QL =0`). The suffix "`QL`" corresponds to the name of the treated score.
...
Only one supplementary parameter as compared to `TTD()` function is available: the parameter `ref.def` in which you can specify the notion of "definitive deterioration" according to the three proposed definition in section 2 :
...
By default, the deterioration is defined as a deterioration with a k-point MCID as compared with the baseline score with no further improvement of of more than k points as compared to the baseline score [@bonnetain2010time]. The result of the application of this function is fairly similar to the one of the `TTD`. However, for `TUDD`, the value of the MCID is also specified in the variables name `time` and `event`. 
...
Like we did for the time to deterioration, all the sensitivity analyses can be performed simultaneously with the primary definition of TUDD. Moreover, many minimal clinically important difference (MCID) can be specified. An indicator of the MCID value is added as a suffix of the resulting parameters `event` and `time`:
...
In this application, death has not been taken into account. Only two sensitivity analyses were performed: the first one regarding patients with no baseline or no follow-up measure and the second one on the MCID value.
...
`xlab` and `ylab` correspond to the name of the abscissa and ordinate axis. The Table 4 summarizes the arguments of this function.
